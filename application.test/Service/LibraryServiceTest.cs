...
public class LibraryServiceTests
{
	...
	public LibraryServiceTests()
	{
		...
		mockBookRepository.Setup(repo => repo.Add(It.IsAny<Book>())).Callback<Book>(book => _bookList.Add(book))
			.Returns(true);
		...
		mockMemberRepository.Setup(repo => repo.GetById(It.IsAny<int>()))
			.Returns((int memberId) => _memberList.Find(m => m.Id == memberId));
	...
	{
		// Arrange
		_bookList = [new Book() { Id = 1, Title = "Book 1", Author = "Author 1" }];
